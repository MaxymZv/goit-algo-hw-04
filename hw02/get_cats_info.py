...
def cats_info(file_path: str): # створюємо функцію cats_infо, яка приймає шлях до файлу

    cats_info = [] # створюємо пустий словник для зберігання інформації
            ...
            for line in f: # проходимось по кожному рядку
                ...
                if len(cat_data) == 3: # перевіряємо чи рядок містить 3 елементи
                    cat_dict = {'id': cat_data[0], # створюємо словник з інформацією про котів
                                 'name': cat_data[1],
                                 'age': cat_data[2]}
                    cats_info.append(cat_dict) # додаємо словник до списку
...
print(cats_info('hw02/cats.txt')) # викликаємо функцію з файлом cats.txt
